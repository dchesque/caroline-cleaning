...
interface TabOutboundProps {
    webhooks: WebhookConfig[]
    onSelect: (webhook: WebhookConfig) => void
}

export function TabOutbound({ webhooks, onSelect }: TabOutboundProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center border-b pb-6">
                <div>
                    <h3 className="text-xl font-serif text-zinc-900">{webhooksT.list.outboundTitle}</h3>
                    <p className="text-sm text-zinc-500 mt-1">{webhooksT.list.outboundSubtitle}</p>
                </div>
            ...
            {categories.length > 0 ? (
                ...
                <div className="py-20 text-center border-2 border-dashed border-zinc-100 rounded-3xl">
                    ...
                    <p className="text-zinc-500 font-medium">{webhooksT.list.empty} "{search}"</p>
                    <button
                        onClick={() => setSearch('')}
                        className="mt-2 text-brandy-rose text-sm font-bold hover:underline"
                    >
                        {webhooksT.list.clearSearch}
                    </button>
                </div>
            )}
        </div>
