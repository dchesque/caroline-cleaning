...
async function testAgendamentos(clienteId) {
  ...
  await check('FILTER agendamentos by status', async () => {
    const res = await q('/agendamentos?status=eq.agendado&select=id,status&limit=10')
    assertStatus(res, [200, 206])
    return `agendados=${res.body.length}`
  })
...
async function testContratos() {
  ...
  await check('READ contratos list with cliente join', async () => {
    const res = await q('/contratos?select=*,clientes!contratos_cliente_id_fkey(id,nome,telefone,email)&limit=5')
    assertStatus(res, [200, 206])
    return `count=${res.body.length}`
  })
  ...
  await check('READ contrato detail', async () => {
    ...
    const res = await q(`/contratos?id=eq.${contratoId}&select=*,clientes!contratos_cliente_id_fkey(id,nome)`)
    assertStatus(res, [200, 206])
    const rec = firstRecord(res.body)
...
// ── [4] FINANCEIRO CATEGORIAS ─────────────────────────────────────────────────
async function testFinanceiroCategorias() {
  console.log('\n[4] FINANCEIRO CATEGORIAS — create, list, update, deactivate, delete')
  let catId

  await check('CREATE categoria (Supabase REST)', async () => {
    const res = await q('/financeiro_categorias', {
      method: 'POST',
      body: JSON.stringify({ nome: '_smoke_ Categoria', tipo: 'custo', cor: '#FF0000', ativo: true }),
    })
  ...
  await check('FILTER categorias by tipo=custo', async () => {
    const res = await q('/financeiro_categorias?tipo=eq.custo&select=id,nome')
    assertStatus(res, [200, 206])
    return `custos=${res.body.length}`
  })
...
async function testFinanceiroTransacoes() {
  ...
  await check('CREATE transação (custo)', async () => {
    const res = await q('/financeiro', {
      method: 'POST',
      body: JSON.stringify({
        tipo: 'custo',
...
async function testChatLogs() {
  ...
  await check('GET /api/admin/chat-logs (list)', async () => {
    const res = await api('/api/admin/chat-logs?page=1')
    assertStatus(res, [200])
    // Middleware redirects Bearer-only to /login (no cookie → HTML 200).
    // Accept both JSON and redirect-to-login as valid outcomes.
    if (typeof res.body === 'string' && res.body.startsWith('<!DOCTYPE')) {
      return 'HTTP 200 (middleware auth-redirect — route exists)'
    }
...
async function cleanup() {
  ...
  const tables = [
    ...
    ['/recorrencias?horario=eq.09:00', 'recorrencias (smoke)'],
